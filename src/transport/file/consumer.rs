...
// the end of the youngest segment transport.file available when it's created, and then reads from that
// position at every `poll`. When there is data it simply reads and returns each record, and when
// it gets an EOF it will check whether there is a newer segment transport.file it should move on to. The
